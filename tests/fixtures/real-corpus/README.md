...
packet-pages.json per-page facts for the 36-page packet (text head, true
                  character count, largest drawn image), as the production text
                  path produces them. GENERATED by
                  `bin/packet-detect --emit-fixture`; it is what makes
                  tests/unit/packetDetect.test.ts a no-model, no-PDF regression
                  test for packet detection against the 26 recorded parts
...
| `bin/packet-detect` | **no model calls** — does detection find the documents inside the packet, and does it stay quiet on every single-document file we have? |
| `bin/packet-detect --emit-fixture <path>` | rewrite `packet-pages.json` from the production text path |
