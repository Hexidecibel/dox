...
import m0114 from '../../migrations/0114_spec_unmatched_ignores.sql?raw';
import m0115 from '../../migrations/0115_document_export_links.sql?raw';
import m0116 from '../../migrations/0116_document_export_link_revocation.sql?raw';
import m0117 from '../../migrations/0117_queue_text_page_sources.sql?raw';
import m0118 from '../../migrations/0118_packet_split.sql?raw';
...
export async function cleanTables(db: D1Database): Promise<void> {
  const tables = [
    ...
    // FKs tenants and clears ahead of it.
    'module_visibility', 'owner_labels', 'tenant_modules',
    // 0091 renewal routing: the ledger FKs documents + tenants and the routes
    // FK users + tenants, so both clear ahead of all three.
    'renewal_alert_state', 'owner_routes',
    ...
    // 0114 dismissed spellings: FK tenants + users only, and nothing points at
    // them, so they clear beside the analytes they are a worklist for.
    'spec_unmatched_ignores',
    // 0115 export links: FK tenants + users only, and nothing points at them,
    // so they clear before the documents whose ids they froze into JSON.
    ...
    // 0112 supplier-list import runs: FK tenants only; derived rows point at a
