...
- **Spec Limits + Out-of-Parameter Warnings**: Acceptance limits on COA test results (`spec_tests` + `spec_limits`, migration 0084). Two sources judge every result — the COA's own printed spec/pass-fail (no configuration, works on every supplier) and OUR configured limit, which is often tighter than what the supplier certifies against. **Three-state by design**: `in_spec` / `out_of_spec` / `not_checked`, where `not_checked` means we held a limit and could not honestly apply it (a censored `<50` against a ≤10 limit, a CFU/mL result against a CFU/g limit) and is never a silent pass. Engine is `shared/specCheck.ts` (pure); review-queue surfacing via `functions/lib/spec-warnings.ts`; register + alerts via `functions/lib/spec-register.ts` (one email per document; routing now delegates to the shared ladder in `functions/lib/alert-routing.ts` — owner route, then `assignments`, then org_admins, with the admin fallback passed in explicitly as `adminFallback: true` because a one-shot safety event reaching nobody is worse than a redundant email). Warns, never blocks. Preview what a limit would catch with `bin/recheck-spec-limits --tenant <id>`. **Criticality (migration 0095)** ranks each limit — `high`/`medium`/`low`, default `medium`, vocabulary owned solely by `shared/specCriticality.ts` and still provisional. It orders and colours the admin list, the review-queue banner/chip/row markers and the alert email, and is frozen into the snapshot; it changes no verdict and hides no result. **Unit equivalence (migration 0093)** is the one per-tenant setting that changes a verdict: off by default, it lets CFU/mL be judged against a CFU/g limit for fluid tenants, names itself in every reason it produces, and is set on Settings › Spec Limits. **Conversions are shown, never silent** (AJ, 2026-09-14): every verdict whose printed magnitude was scaled (`sample_basis`, e.g. CFU/100g → CFU/g, cfu/0.1g → CFU/g) or equated (`tenant_volume_mass`) carries `conversion {from, to, rule, factor, operation}`, frozen into `limit_snapshot` and rendered as a "Converted: …" chip on the value (`ConversionChip`); product-dependent comparisons (per mL vs per g without the setting, % w/w vs % v/v always) are `not_checked` with the rule named in the reason. **Supplier watch + completeness (migration 0107)**: a COA is complete by default; `supplier_required_analytes` (per supplier + document type, managed at `/api/spec-required-analytes`, UI on Settings › Spec Limits "On watch" and the supplier page) makes one incomplete, reported as `missing_required` (`checkRequiredAnalytes`, same exact name/alias matching as limits, per record) — never a pass, never `not_checked`. Supplier-scoped limits over company limits are the watch; `review_by` on either never loosens anything, it flags "watch period ended <date> — review" (`watchStatus`, `overdueWatches`). **Four states on a printed result**: `in_spec` / `out_of_spec` / `not_checked` (held a limit, could not apply it) / `unjudged` = "No limit configured" (no limit in scope AND no printed spec), rendered in a distinct neutral grey in the review queue (row chips + banner), the Out of Spec register (Incomplete / No limit configured tabs) and the document page's Test results panel (`DocumentSpecResults`); could-not-check is blue and prefixed "Could not check — verify", incomplete is its own purple. No report export includes spec results yet. At approval both non-judgements go to `document_spec_gaps` (read at `/api/spec-gaps`), and missing required analytes ride the SAME one-per-document spec alert email (`notifySpecFailures`), never a second send path. Setup helper: `bin/seed-supplier-watch` (dry run default).
...
| 0107 | supplier_spec_watch | **A supplier on watch, and what a certificate did not say** (AJ rulings, 2026-09-14). Three changes. (1) `supplier_required_analytes` (tenant, supplier, document_type, spec_test — **all NOT NULL**, plain UNIQUE; + `effective_from`, `review_by`, `reason`, created/updated by): an analyte this supplier's certificates of this type MUST report. A COA is **complete by default**; only these rows make one incomplete. `document_type_id` is NOT nullable although `spec_limits` scopes type with NULL = any: "any type" would flag the same supplier's insurance certificate as missing coliform, and a nullable key reopens 0086's NULL-distinctness trap. No `active` flag — a watch ends by an audited delete. (2) `spec_limits.review_by` (YYYY-MM-DD, supplier-scoped limits only — the API refuses it tenant-wide). **Not an expiry**: after it passes the tighter limit/requirement STILL APPLIES and is flagged "watch period ended <date> — review" until an admin extends or removes it; frozen into `limit_snapshot` with `review_overdue`. (3) `document_spec_gaps` (`kind` 'missing_required' / 'unjudged'): what was NOT judged at approval. **A separate table, not a new `document_spec_checks` verdict** — 0085's verdict CHECK cannot be altered without rebuilding the register and 0105's partial unique index on prod, and a register row is a judgement of a printed result (value, limit, acknowledgement) that neither kind has. Identity: analyte for missing_required, `result_key` for unjudged, enforced by paired CHECKs so the expression UNIQUE on `COALESCE(result_key, spec_test_id)` never sees NULL. `required_analyte_id` is not an FK; `snapshot` freezes the requirement. **May need renumbering** if a parallel 0107 lands first |
