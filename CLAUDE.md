...
- **Exact-Duplicate Intake (migration 0108)**: a file byte-identical (SHA-256, same tenant) to one already here does not become a second Review Queue card. The rule lives once in `functions/lib/intake/duplicates.ts` and runs inside `enqueueDocument`, so smart upload, both email webhooks (the legacy one calls `admitIntake` directly, before any model runs), connector drop/run/retry, the S3 poller and the supplier portal all share it. **Identical to an APPROVED file** (a live document version with that checksum, OR an approved queue item — page-scoped sublot documents carry their own pages' checksum, and order/shipment approvals have no document) → an `intake_duplicates` row links the arrival to that document, shown on the document page as "Received again". **Identical to a file still WAITING** → a row against that queue item, whose card says "also received from …". **Identical to a REJECTED file** → queued normally, card says "this exact file was rejected on … for …" (`intake_history.previously_rejected`, computed at read). Nothing is deleted or rejected: the bytes stay, the exact enqueue call is stored, and **Review anyway** (`POST /api/intake-duplicates/:id/review`, Review Queue › Received again) replays it and audits. Portal: the supplier's response is unchanged; case 1 sets `request_uploads.document_id` (arrival `document_linked`, decided on Arrivals as usual), case 2 points `queue_id` at the waiting item and every arrival on it is linked at approval. `POST /api/documents/ingest` is deliberately NOT checked — it is a caller-asserted upsert, not a queue door. Byte-identical only; near-duplicate re-scans are out of scope. Existing surplus copies: `bin/audit-duplicate-documents [--remote]` reports groups (first vs later copy, with every link hanging off each) and changes nothing.
...
| 0107 | product_identifiers | **What a product goes by, and who said so.** One row per identifier of OUR product: `kind` ('our_sku' / 'supplier_item' / 'supplier_name' / 'alias' / 'gtin' / 'pack'), `value` + `value_norm` (computed in `functions/lib/product-identifiers.ts`, never SQL; codes keep leading zeros), `supplier_id`, `superseded` (a former number), `confirmed`, `source` ('seed' / 'reviewer' / 'extracted' / 'import', CHECKed), `note` (the evidence), created/confirmed by+at. **Not `supplier_product_map`** (0075): its UNIQUE on the supplier product NAME cannot hold CMF's one name on two items (30904 tote = our 10286, 50903 bag = our 0801), and the matching engine reads it. **Not `product_suppliers.supplier_sku`**: one SKU per product per supplier, no provenance. A CHECK pairs kind with supplier_id both ways (supplier kinds NOT NULL, the rest NULL); the NULL-distinctness trap is answered with TWO PARTIAL unique indexes (with / without a supplier), each over NOT NULL keys. Uniqueness is per product, not tenant: two products claiming one SKU is ambiguity search must SHOW. Removal is a DELETE with the whole row in the `product_identifier.removed` audit row. No order index: `orders` already has UNIQUE(tenant_id, order_number) for the A7 seek |
| 0108 | intake_duplicates | **A file we have already seen does not become a second review.** Intake stored `processing_queue.checksum` on every door and nothing compared it; connectors deduped by R2 PATH, which a forwarded email defeats, so the same attachment forwarded twice became two approved documents (prod: 61 same-file groups / 74 later copies by `bin/audit-duplicate-documents`). `intake_duplicates` is the ledger of SUPPRESSIONS only — `match_kind` `already_approved` (matched_document_id, or matched_queue_id for an order/shipment approval) / `already_waiting` (matched_queue_id). A resend of a REJECTED file is queued, not recorded here, so every row is "did not reach a reviewer". `enqueue_params` (JSON) + `file_r2_key` make it replayable; `queue_id IS NULL` = still suppressed, set by Review anyway with `overridden_by`/`_at` (claimed before enqueueing so a double click cannot queue twice). Every pointer ON DELETE SET NULL and no CHECK tying kind to pointer, because SET NULL would then make a parent delete fail. Also indexes `processing_queue(tenant_id, checksum)` and `document_versions(checksum)` — never indexed because never read. No unique index on pending checksums: prod already holds pending twins, so two identical files in the same instant can both pass (one request wide). **Built as 0107 alongside the product-identifier branch, which took that number first; renumbered to 0108 at merge** |
