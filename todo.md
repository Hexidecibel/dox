...
- **Spec limits — ship migration 0086 + the Pages deploy TOGETHER.** 0086 (scope uniqueness index) is applied locally and deliberately NOT on prod: it turns a duplicate-scope create into a constraint violation, and the 409 handling that makes that a friendly error is committed but undeployed. Applying the index alone puts a 500 on prod. Needs a Cloudflare token with `Cloudflare Pages : Edit`.
- **Spec limits — gap detection ("we hold a Listeria limit, this COA never reported one").** Does not exist anywhere; `unmatched` is strictly the reverse direction. This is THE finding for a supplier under a sanitation alert: Andersen tests for two organisms and returns a clean register by default. Needs an "expected analytes per supplier" concept — a naive version fires on every COA. **Gated on AJ's Q4** in `~/drops/aj-micro-limits-questions.md`.
...
- **Spec engine — `applyRowUnit` breaks scientific notation.** It appends the unit column to the value, so `"3.0x10^2"` + `"CFU/g"` parses as **3, not 300** — a silent `in_spec` on a value 3× over its limit. Pre-existing, found 2026-09-01, not fixed.
