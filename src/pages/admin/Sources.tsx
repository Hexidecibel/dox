...
interface Connector {
  ...
  updated_at: string;
  tenant_id: string;
  /** Source identity facets — surfaced as an "Origin · Output" chip. */
...
export function Sources() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editingConnector) {
        await api.sources.update(editingConnector.id, data);
      } else {
        ...
        await api.sources.create({ ...data, tenant_id: createTenantId });
      }
      setDialogOpen(false);
      loadConnectors();
    } catch (err) {
  ...
  const handleDelete = async (connector: Connector) => {
    if (!confirm(`Delete source "${connector.name}"?`)) return;
    try {
      await api.sources.delete(connector.id);
      loadConnectors();
    } catch (err) {
  ...
  return (
    <Box>
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 3, flexWrap: 'wrap', gap: 1 }}>
        ...
        <Button variant="contained" startIcon={<AddIcon />} onClick={() => navigate('/admin/sources/new')}>
          Add Source
        </Button>
      </Box>
          ...
          {connectors.length === 0 ? (
            ...
            connectors.map((connector) => (
              <Card
                ...
                variant="outlined"
                sx={{ cursor: 'pointer' }}
                onClick={() => navigate(`/admin/sources/${connector.id}`)}
              >
      ...
      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} maxWidth="sm" fullWidth fullScreen={isMobile}>
        <DialogTitle sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          {editingConnector ? 'Edit Source' : 'Add Source'}
        ...
        <DialogActions sx={{ px: 3, pb: 2 }}>
          ...
          >
            {saving ? 'Saving...' : editingConnector ? 'Save Changes' : 'Add Source'}
          </Button>
        </DialogActions>
      </Dialog>
    </Box>
