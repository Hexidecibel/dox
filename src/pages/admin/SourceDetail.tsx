...
interface Connector {
  ...
  /** Source identity facets — used for the "Origin · Output" header chip
   * and to gate the extraction-mapping editor (which keys on the
   * supplier + document type, not the connector row). */
...
function normalizeConnector(raw: unknown): Connector {
  ...
  return {
    ...
    origin_kind: (r.origin_kind as string | null) ?? null,
    output_kind: (r.output_kind as Connector['output_kind']) ?? null,
    supplier_id: (r.supplier_id as string | null) ?? null,
    document_type_id: (r.document_type_id as string | null) ?? null,
  };
}
 ...
 * fired through `api.sources.patch` with `extraction_instructions`.
 *
...
function ConnectorExtractionInstructionsEditor({
  ...
  const save = async (text: string) => {
    ...
    try {
      await api.sources.patch(connectorId, { extraction_instructions: text });
      setPersisted(text);
      setSaveState('saved');
      onSaved(text);
      setTimeout(() => {
...
export function SourceDetail() {
  ...
  const loadConnector = useCallback(async () => {
    ...
    setError('');
    try {
      const result = (await api.sources.get(id)) as { connector: unknown };
  ...
  const loadRuns = useCallback(async () => {
    ...
    setRunsLoading(true);
    try {
      const result = (await api.sources.listRuns(id, {
        limit: RUNS_PER_PAGE,
  ...
  const loadHealth = useCallback(async () => {
    if (!id) return;
    try {
      const result = await api.sources.health(id);
      setHealth(result);
    } catch {
      setHealth(null);
    }
  ...
  useEffect(() => {
    ...
    const message = address
      ? `Source created. Drop a file in the upload zone above to test, or send email to ${address}.`
      : 'Source created. Drop a file in the upload zone above, or use the receive address card below.';
  ...
  const patchConnector = useCallback(
    async (partial: Record<string, unknown>, optimistic?: Partial<Connector>) => {
      ...
      try {
        const result = (await api.sources.patch(id, partial)) as { connector: unknown };
  ...
  const handleTest = async () => {
    ...
    setError('');
    try {
      const result = (await api.sources.test(id)) as {
        success: boolean;
        message: string;
        warnings?: string[];
        probe?: {
  ...
  const handleRetryRun = useCallback(
    async (runId: string) => {
      ...
      setError('');
      try {
        const result = await api.sources.retryRun(id, runId);
  ...
  const commitFieldMappings = async (next: ConnectorFieldMappings) => {
    if (!connector) return;

    // Connectors → Sources: the worker reads field_mappings from the
    // (supplier_id, document_type_id) EXTRACTION PROFILE in
    // supplier_extraction_instructions — NOT the connector row. So when this
    // source is keyed to a supplier + document type, edits here must persist
    // to that profile (matching what SourceWizard's Review Schema step does),
    // otherwise they silently never reach the worker. Only the whole-stream
    // case (no supplier/doctype, e.g. an internal/order source) still saves
    // the mappings on the connector row, which is what the worker falls back
    // to when there's no profile key.
    if (connector.supplier_id && connector.document_type_id) {
      const prev = connector;
      // Optimistic UI update so the editor reflects the change immediately.
      setConnector({ ...prev, field_mappings: next });
      try {
        const scope = prev.tenant_id ? { tenant_id: prev.tenant_id } : {};
        // Fetch current natural-language guidance first so the mandatory
        // `instructions` field on the PUT doesn't clobber it.
        ...
        setSaveSnack('Field mappings saved to extraction profile');
      } catch (err) {
        setConnector(prev);
    ...
    // No supplier/doctype: whole-stream mappings live on the connector row.
  ...
  if (!connector) {
    return (
      <Box sx={{ py: 4 }}>
        ...
        <Button startIcon={<BackIcon />} onClick={() => navigate('/admin/sources')} sx={{ mt: 2 }}>
          Back to Connectors
        </Button>
      </Box>
    );
  ...
  return (
    <Box>
        ...
        >
          <Box sx={{ flex: 1, minWidth: 0 }}>
            ...
            <Stack direction="row" spacing={1} flexWrap="wrap" useFlexGap sx={{ mb: 1 }}>
              ...
              />
              <Chip
                label={sourceKindLabel(connector.origin_kind, connector.output_kind)}
                size="small"
                variant="outlined"
              />
      ...
      <Paper variant="outlined" sx={{ p: 3, mb: 3 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5, mb: 0.5 }}>
          ...
          <Box
            ...
            role="button"
            tabIndex={0}
            aria-label="Drop a file to run the source"
            onKeyDown={(e) => {
      ...
      {/* prompt on every run. The prompt helpers now live in              */}
      {/* shared/orderPrompt.ts (consumed by bin/process-worker).          */}
      <Paper variant="outlined" sx={{ p: 3, mb: 3 }}>
        <Box sx={{ mb: 2 }}>
          <Typography variant="h6" fontWeight={600}>
...
function ApiDropCard({
  ...
  const endpoint = useMemo(() => {
    ...
    return `${origin}/api/sources/${handle}/drop`;
  ...
  const performRotate = async (isBootstrap: boolean) => {
    setRotating(true);
    try {
      const result = await api.sources.rotateApiToken(connector.id);
      ...
      <Box sx={{ mb: 2 }}>
        ...
        <Stack direction="row" spacing={1} alignItems="center">
          <TextField
            ...
            inputProps={{ 'aria-label': 'Source drop endpoint URL' }}
          />
          ...
          <>
            <Stack direction="row" spacing={1} alignItems="center">
              <TextField
                ...
                value={tokenDisplay}
                InputProps={{ readOnly: true, sx: { fontFamily: 'monospace' } }}
                inputProps={{ 'aria-label': 'Source API bearer token' }}
              />
...
function S3DropCard({
  ...
  const performProvision = async () => {
    ...
    try {
      const creds = await api.sources.provisionR2(connector.id);
  ...
  const performRotate = async () => {
    setRotating(true);
    try {
      const creds = await api.sources.rotateR2(connector.id);
      setRevealedSecret(creds.secret_access_key);
      setShowSecret(true);
      setConfirmRotateOpen(false);
      onRotated(creds);
...
function PublicLinkCard({
  ...
  const performGenerate = async (
    ...
    setBusy(true);
    try {
      const result = await api.sources.generatePublicLink(connector.id, {
  ...
  const performRevoke = async () => {
    setBusy(true);
    try {
      await api.sources.revokePublicLink(connector.id);
      onRevoked();
