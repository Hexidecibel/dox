...
export function ConnectorRunReview() {
  ...
  const load = useCallback(async () => {
    ...
    setError('');
    try {
      const result = await api.connectors.runs.staged(connectorId, runId);
      setData(result);
  ...
  if (!data) {
    return (
      <Box sx={{ py: 4 }}>
        ...
        <Button
          startIcon={<BackIcon />}
          onClick={() => navigate(`/admin/connectors/${connectorId}`)}
          sx={{ mt: 2 }}
        >
          Back to connector
        </Button>
...
export default ConnectorRunReview;
