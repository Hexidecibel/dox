...
export const vendorDocsContent: VendorDocsContent = {
  pageTitle: 'Sending files to a dox source',
  intro:
    "A dox source is the inbound channel for files going to a tenant. Whoever shared this page with you set one up and picked a slug for it (the URL-safe handle baked into every delivery address). You can deliver files through any of the doors below — pick whichever fits your tooling. All five doors land the file in the same place; the tenant on the other side will see your upload regardless of how it arrived.",
  doors: [
    {
      id: 'email',
      title: 'Email',
      intro:
        "Send the file as an attachment to the per-source inbox. Best if you already email reports as PDFs / CSVs / XLSX and just want them to land in dox automatically.",
      example:
      ...
      gotchas: [
        ...
        "Subject lines are kept for your reference but are not used for routing — every email to <slug>@supdox.com hits the same source.",
        "If the source owner has set sender or subject filters, only matching emails will be processed. Ask them which filters are in place if your test emails get dropped silently.",
      ],
    },
    {
      id: 'api',
      title: 'HTTP API',
      intro:
        "POST a multipart file body to the source's drop endpoint with a bearer token. Best if you already have an automation script (curl, Python requests, Node fetch, etc.) emitting files on a schedule.",
      example:
        'curl -X POST \\\n' +
        '  -H "Authorization: Bearer <your-bearer-token>" \\\n' +
        '  -F "file=@/path/to/orders.csv" \\\n' +
        '  https://supdox.com/api/sources/<connector-slug>/drop',
      gotchas: [
        "If you start getting 401 Unauthorized, the source owner rotated the token. Ask them for the new one — the old token is dead the moment they rotate.",
        "The `file` field name is required. The endpoint reads the first form-data file under that exact key.",
        "On success the endpoint returns 202 Accepted with a JSON body containing the run id. Use that id to correlate with the source owner if anything goes wrong.",
    ...
    },
    {
      ...
      title: 'S3 bucket',
      intro:
        "Upload to a per-source S3-compatible bucket using vendor-specific credentials. Best if you already use AWS / S3 tooling (aws-cli, rclone, boto3) and want to drop files without writing new HTTP code.",
      detail:
        "The bucket is hosted on Cloudflare R2, which speaks the S3 protocol. Any tool that lets you point at a custom endpoint URL will work — just plug in the values the source owner gave you.",
      example:
      ...
      gotchas: [
        ...
        "The secret key is shown only once when the source owner provisions or rotates it. If you lost it, you need a new pair — the old secret is not recoverable.",
    ...
    },
    {
      ...
      gotchas: [
        "The URL is the credential. Treat it like a password — anyone who has the URL can upload to this source. Don't post it in public chats or commit it to a repo.",
        "Links expire (default 30 days). If you see 'This link is no longer active,' the link was revoked or expired — ask the source owner for a new one.",
    ...
    },
    {
      ...
      intro:
        "If you have access to the dox admin UI for the tenant on the other side, open the source's detail page and drop a file directly into the upload zone. Most vendors won't have admin access — this door is mainly for the tenant's own staff testing the source.",
      example: '(Sign in to dox, navigate to the source, drag a file onto the upload zone.)',
      gotchas: [
  ...
  footer:
    'Need help? Contact the person at the tenant who shared this page with you — they own the source and can rotate credentials, extend a public link, or check why a delivery did not land.',
};
