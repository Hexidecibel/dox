...
1. Use these EXACT canonical field names (snake_case):
   ...
   - expiration_date — the PRODUCT's expiration, best-by, use-by, or sell-by DATE, as printed. It is the day the material stops being good, and it says nothing about how long the paperwork is valid. A stated PERIOD is NOT this field — "18 months from manufacture", "14 days at or below 41 F" is shelf_life, and it is never converted into a date here.
   ...
   - shelf_life — the PRODUCT's life as a PERIOD, copied exactly as printed, together with the storage condition when the page states one ("18 months from date of manufacture", "14 days at or below 41 F", "6 months frozen, 10 days thawed"). Labels include "Shelf Life", "Shelf life/Storage", "Product Life", "Best if used within"; it is also printed unlabelled in a specification's storage block or critical-analysis grid. IT IS A DURATION AND NEVER A DATE: do not convert it, do not count it forward from any date on the page, and never put it in expiration_date or document_expires_on. When the page states more than one life for different storage conditions, keep ALL of them in this one string as printed — a multi-condition shelf life reduced to a single number is a claim the page does not make. Rule 7 governs: if the page states no shelf life, this is null.
   ...
   - certificate_number — the certificate's OWN number, as printed and labelled ("Certificate No.", "Cert #", "Certificate ID"). A policy number, a form number, an audit report number, a customer's item number and a document revision number are NOT certificate numbers. A certificate that lists several PRODUCTS with a per-product code in a table ("Product Certificate #", "Halal-ID") does not state its own number there: the certificate's number is the document-level one in the header block, including when that line is labelled "Document #" — never a table cell (table rule 12). When the page prints none, this is null however many other numbers it prints.
   - document_number — the identifier THIS PIECE OF PAPER carries for itself: a document, specification, form or procedure number as printed ("Document No.", "Doc #", "Spec No.", "Specification No.", "Form No.", "SOP #"), including an unlabelled one printed in a controlled-document header or footer beside a standard's clause reference and a revision note. THIS FIELD IS FOR A DOCUMENT THAT IS NOT A CERTIFICATE: on a certificate, the number that identifies the paper is certificate_number even when the page labels it "Document #" or "Doc #", and this field is then null. It is NOT the lot or batch number, NOT a PO, order, invoice or sales-order number, NOT the supplier's product or item code, NOT the customer's item number, NOT a plant, facility, UPC or phone number, and NOT the revision or version printed next to it ("rev 9") — a revision's DATE is revision_date. A file name or file path printed in a footer says where the document is STORED, not what it is called: a value containing a path separator, a drive letter or a file extension is NEVER a document number. Rule 7 governs: if the page prints no number of its own, this is null however many other numbers it prints.
...
2. For dates: normalize to YYYY-MM-DD. Two-digit years mean 2000s (e.g., '26 = 2026, 03/08/26 = 2026-03-08). If ambiguous, keep as-is. A date printed only as a CODE — a Julian date or a lot code (e.g. "6094", "10426212") — is NOT a calendar date: never decode or convert it. Copy the code exactly as printed into the field it is labelled as, and leave every date field empty unless the page prints that date. Decoding a lot code is done later, from the supplier's declared lot format, where it is labelled and checked.
...
const FIELD_ALIASES: Record<string, string[]> = {
  ...
  // The PRODUCT's life as a PERIOD (2026-09-17). Deliberately NO 'best_by',
  // 'expiry' or 'exp_date' here: those are DATES and belong to expiration_date,
  // and folding a date onto this field would hand the renewal ladder a shelf
  // life wearing a period's name. KEEP IN SYNC with the other copy.
  shelf_life: ['shelf_life_days', 'shelflife', 'product_shelf_life', 'product_life', 'storage_life'],
  // The paper's OWN identifier. Narrow on purpose: 'number', 'id', 'reference'
  // and 'form' alone can mean anything on these documents, and certificate_number
  // owns a certificate's own number.
  document_number: ['document_no', 'doc_number', 'doc_no', 'spec_number', 'spec_no', 'specification_number', 'document_id', 'form_number', 'sop_number'],
