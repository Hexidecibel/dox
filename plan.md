...
**Status:** done (local, migration 0114; not deployed)
...
- `POST /api/document-exports/send` — a token-gated link, never attachments,
  ...
  if the send fails.
- Migration 0114 `document_export_links` + `/export/:token`
...
- **Done (local, 2026-09-15; migration 0109, not deployed) — supplier watch, AJ rulings 2026-09-14:**
...
### Supplier requirements from real data — packets, worklist, verified supplier list import (AJ 2026-09-14)

**Status:** done — built locally 2026-09-15 (migration 0112, renumbered from 0111 on merge; not deployed)

**Summary:** Every live supplier carried the same invented requirement set (294 rows = 21 x 14, all `source IS NULL`). AJ's minimum for every approved supplier: certificate of insurance, a third-party food safety certificate, claim paperwork where a claim is made, spec sheets for products bought; everything else derived from the verified supplier list. Three pieces, one rule function:

1. **Packets from the admin screens.** Supplier Requirements › Suppliers (select many) and Supplier › Requirements (one) → `ApplyPacketDialog`, preview first. `POST /api/supplier-requirements/apply-packet` (named `supplier_ids`, cap 200, `dry_run` default true): no row → add (`source='packet'`, `packet_slug`); unconfirmed (NULL) row → adopt at the packet tier; human / packet / derived row → untouched and reported with both tiers; `replace_unconfirmed` also removes the supplier's NULL rows the packet does not name. One `requirement_packet.apply` audit row per changed supplier. Idempotent. `GET /api/supplier-requirements/packets` (tenant's wizard pack, else fsqa).
2. **Needs review worklist.** `GET /api/supplier-requirements?review=unconfirmed|flagged|any`; `POST /api/supplier-requirements/review` confirm (→ `source='human'`, flag cleared, `supplier_requirement_confirmed` audit) / remove (hard delete as the editor's detach, `supplier_requirement_deleted` audit carrying the row; refuses person-set rows); "Replace with a packet" = apply-packet with `replace_unconfirmed`. A person's POST / PUT-tier now stamps `source='human'` (previously NULL, which would have put hand-added rows in the worklist).
3. **Verified supplier list import.** Template (`shared/supplierListTemplate.ts`): Supplier name*, Supplier contact email, Supplier category* (ingredient / packaging / chemical-sanitation / distributor / co-packer), Approved (Y/N)*, Product SKU, Product name, Claims made. One row per (supplier, product). `POST /api/supplier-list/import` (`csv` | `xlsx_base64` | `rows` | `rerun_of`; `dry_run` default true) → `runSupplierListImport` → `deriveSupplierRequirements` (pure). Rules: baseline `certificate-of-insurance` + `third-party-audit-certificate` (required) for every approved supplier; category packet (ingredient & co-packer → ingredient-supplier, chemical-sanitation → chemical-sanitation, packaging & distributor → none, reported); each claim on a product → tenant `claim_type_requirements` at the rule tier; any product bought → `spec-sheet` required; not approved → nothing; required beats recommended; unknown slugs reported. Writes: add `source='derived'` + `derivation_run_id` + `derivation_basis`; adopt NULL rows (but never LOWER a NULL row's tier by adoption: such a row is held unconfirmed in the worklist and shown with both tiers, e.g. the seed's required Letter of Guarantee vs the rBST-free rule's recommended); refresh derived rows; never touch human/packet rows; derived rows the list no longer implies → `review_flag='not_on_verified_list'`, never deleted, cleared when supported again. Runs stored in `supplier_list_imports` (counts, row outcomes, input rows); `supplier_list.import` audit. Suppliers matched/created via `findOrCreateSupplier` (dry run: read-only resolver, `will_create`). Products matched via `product_identifiers` / product name (unmatched = warning; spec sheet still owed). Claims matched to claim types by slug/name (unmatched = reported, nothing derived). Module: `/api/supplier-list` under `library`.

**Open for AJ:** is `third-party-audit-certificate` ("3rd Party Audit CERTIFICATE") his "third-party food safety certificate"? co-packer → ingredient packet? packaging / distributor packets (none exist)? Is a supplier list import the WHOLE list (current: yes — absent suppliers' derived rows get flagged)? Spec sheet is per supplier, not per product (`supplier_requirements` has no product dimension).

**Deferred:** per-product spec sheet obligations; writing supplier contact emails anywhere (no supplier contact store); creating products / `product_identifiers` (source 'import') from the list; moving the derivation rules into the pack JSON; a webhook caller; a worklist filter "unconfirmed but supported by the latest list".

**Files:** `migrations/0112_supplier_list_derivation.sql`, `shared/{requirementDerivation,supplierListTemplate}.ts`, `functions/lib/{requirement-derivation,supplier-list-import}.ts`, `functions/api/supplier-list/**`, `functions/api/supplier-requirements/{apply-packet,packets,review,index,[id]}.ts`, `src/components/{ApplyPacketDialog,RequirementsWorklist,SupplierListImport,SupplierRequirementsEditor}.tsx`, `src/pages/admin/SupplierRequirements.tsx`, `tests/unit/{requirementDerivation,supplierListTemplate}.test.ts`, `tests/api/{supplier-list-import,supplier-requirements-packets}.test.ts`, `tests/fixtures/supplier-list/`.
